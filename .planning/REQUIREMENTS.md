...
- [ ] **QUEUE-04**: 工作流模組化為獨立 stage（下載→聽打→校對→排版），各 stage 可並行（下載完第 1 集即交付聽打，同時下載第 2 集）
- [ ] **QUEUE-05**: 失敗任務自動重試（可設定重試次數）

### API (API)

- [ ] **API-01**: RESTful API 端點：建立任務（上傳音檔或提供 YouTube 連結）
- [ ] **API-02**: RESTful API 端點：查詢任務狀態（pending/running/done/failed）
- [ ] **API-03**: RESTful API 端點：取消任務
- [ ] **API-04**: RESTful API 端點：下載任務結果
- [ ] **API-05**: API 認證（JWT token）
...
- [ ] **UI-01**: Mobile-first 響應式設計（手機、平板、PC），手機友善是硬需求
- [ ] **UI-02**: 認證登入（Email/Password + Google OAuth）
- [ ] **UI-03**: 上傳音檔介面
- [ ] **UI-04**: YouTube 播放清單追蹤介面（輸入清單 URL，管理追蹤狀態）
- [ ] **UI-05**: 參數設定介面（Prompt、音檔性質、輸出格式選擇）
- [ ] **UI-06**: 任務進度追蹤介面（即時顯示任務狀態與進度）
- [ ] **UI-07**: 結果下載介面

### Notification & Delivery (NOTF)

- [ ] **NOTF-01**: 任務完成後發送 Email 通知
- [ ] **NOTF-02**: 多格式輸出（txt, srt, word, excel, json）
